...
def process_image(image_path: str, face_data_file_path: str, liveness_threshold: Optional[float] = None) -> str:
    """Process the image to recognize faces and return attendance time with a customizable liveness threshold."""
    ...
    liveness_variance = 0.0  # This will store the liveness check variance
                    ...
                    if not known_face_encodings or not known_face_names:
                        ...
                        if not face_locations:
                            ...
                            for (top, right, bottom, left), face_encoding, landmarks in zip(
                                ...
                                # Check for liveness if the threshold is not None
                                if liveness_threshold is not None:
                                    liveness_variance = liveness_check(upper_face)

                                    if liveness_variance <= liveness_threshold:  # Use user-defined liveness threshold
                                ...
                                if any(matches):
                                    ...
                                    if best_match_accuracy < 0.45:  # Adjusted threshold for masked face recognition
                                        ...
                                        if hour < 12:
                                            message = f"Good morning {best_match_name}, ID: {best_match_id}"
                                        else:
                                            message = f"Good evening {best_match_name}, ID: {best_match_id}"
    ...
    result = {
        ...
        "liveness_variance": liveness_variance
    }

    return to_json(**result)

# Define file paths
image_path = "C:/Users/user/Downloads/DB/osama.png"
face_data_file_path = 'face_data.pkl'

# Call the process_image function
result_json = process_image(image_path, face_data_file_path, 100)  # Example liveness threshold
